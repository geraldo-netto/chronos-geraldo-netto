...
| --- | --- | --- | --- | --- |
| T1043 | open | high | s | Resolve existing output-path ancestors before the source-overlap check in `scripts/package-spices.mjs:228` and recursive removal at `:257`. With `dist -> files`, the default `dist/chronos@geraldo-netto` output aliases the applet source directory but passes the lexical check; a temporary fixture reproduced deletion of the original `metadata.json` followed by `ENOENT`. Reject aliases that would replace source files and cover a symlinked output parent. |
| T1044 | open | medium | m | Replace the fetched year/region's previous holiday snapshot in `files/chronos@geraldo-netto/holidayCache.js:542` instead of only merging new rows. Two successful refreshes currently concatenate an old and corrected name and retain a removed holiday; a subsequent accepted empty response also leaves both dates visible while refreshing their freshness stamp. Preserve other regions and handle overlapping year-spanning records when invalidating the indexes and month memo. |
| T1045 | open | medium | s | Center the current/next agenda row using the scroll adjustment's visible page size in `files/chronos@geraldo-netto/6.0/eventView.js:279`. Subtracting half the entire events container height produces a negative scroll target for a row at y=400 with height 40 in 1000 pixels of content and a 200-pixel viewport; the adjustment clamps to zero and leaves that row off-screen instead of scrolling to 320. Cover content taller than the viewport. |
...
| T1015 | blocked | medium | s | Choose the weather-location contract: preserve an empty value and only prefill the field until commit, or restore the timezone-derived city when settings opens and correct the tooltip. The current constructor persists a city the user did not enter. |
