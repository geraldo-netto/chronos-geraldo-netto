...
| T1103 | open | low | s | Remove the compatibility-only cancelPendingWeatherRequests alias from weatherProviders.js:280 and weather.js exports, updating its tests to call releaseWeatherConsumer. Production already uses the explicit acquire/release API; the old cancellation name misleadingly exposes a non-idempotent reference-count decrement and is retained only by tests. |
