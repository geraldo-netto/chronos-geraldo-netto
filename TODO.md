...
| --- | --- | --- | --- | --- |
| T1159 | open | medium | s | Honor exclusive end dates for timed events ending at midnight. eventData.js:129–138/161–181 and eventIndex.js:236–247 put a timed event from September 8 at 23:30 to September 9 at 00:00 in both days' agenda and dot buckets; a native CJS reproduction confirms the spurious September 9 entry. Compute occupied dates without changing the displayed end time, and preserve zero-duration events. Add permanent regressions for midnight endpoints, ordinary overnight events, all-day events, zero duration, and DST boundaries. |
| T1160 | open | medium | s | Bound calendar-plugin import status text in chronos_settings_widgets_calendars.py:254–256/435. Importing a valid plugin whose name is 100 ASCII W characters expands a native SettingsPage from 720px to 1420px at Sans 10 and 1920px at Sans 14 on a 1366px display because the success Gtk.Label has no width constraint. Wrap or ellipsize status text while retaining its complete accessible text. Add a permanent composed settings regression for successful imports with maximum-length names and verify native widths at normal and enlarged fonts. |
| T1161 | open | medium | s | Preserve focus when incoming agenda rows replace the empty state. eventView.js:227/740 hides the focused No Events/Add-an-event button before transferring focus; a private native Cinnamon reproduction of an asynchronous event arrival closes the popup and moves focus to MetaStage. Park focus on a stable heading before hiding the button, then restore it to an appropriate row without overriding user navigation or stale builds. Add permanent regressions for event and holiday arrivals, chunked rendering, external focus, and native popup persistence. |
| T1162 | open | medium | s | Transfer focus outside the agenda before disabling Show Events. appletCoordinators.js:144 hides the entire focused event column; a private native Cinnamon reproduction of coordinator.apply(false) closes the popup and moves focus to MetaStage. Move agenda-owned focus to the selected calendar day or another stable visible actor before hiding the column. Add permanent regressions covering focused event rows, heading, and empty-state button, preservation of external focus, and native popup persistence. |
| T1163 | open | medium | s | Reject or canonicalize native-unsafe holiday names at provider, cache, and calendar-adapter admission. holidayRecord.js:63/236 and holidayCache.js:70 accept embedded NUL and lone surrogates; a provider or cached name Before\u0000After survives into the month map, then native tooltip text truncates at the NUL and conceals subsequent same-day calendar names. Enforce a consistent Unicode policy while preserving valid fallback sources. Add permanent regressions composing malformed provider/cache names with a valid same-day plugin and checking complete native-safe output or explicit refusal. |
