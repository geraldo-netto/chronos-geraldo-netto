...
| T541 | configuration discoverability | Low | open | S | The country tooltip's privacy disclosure (`5.4/settings-schema.json:163`) says the lookup "sends the selected country to a third-party holiday service" (singular), but since `122d746` the fallback chain can contact up to four services, each receiving the country; README already uses the plural. Reword the tooltip, regenerate the pot, and merge the catalogs. | With CalDays removed the chain contacts up to three services; rewording means one msgid churn across all 15 catalogs, so merge and translate in the same change. |
