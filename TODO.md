...
| --- | --- | --- | --- | --- |
| T1148 | open | medium | s | Settle forecast attempts when a provider's merge, normalize, or timezoneFor hook throws in an asynchronous HTTP callback. weatherProviders.js:543–559 lets these exceptions escape, bypassing healthy fallback providers and leaving WeatherReadingRepository._inflight occupied indefinitely; subsequent refreshes only add subscribers. A real resolver/repository reproduction with a throwing normalizer and healthy second provider delivers nothing and retains two subscribers after another refresh. Catch adapter result-construction errors, report them and fail over while preserving downstream consumer callback exceptions; cover all three hooks and flight cleanup. |
...
| T1151 | open | medium | s | Preserve keyboard focus while rebuilding calendar grid actors. 6.0/calendar.js:385–389 destroys the focused day or header without parking focus. Private native Cinnamon reproductions for week-number visibility, first-day-of-week settings, and the registered LC_TIME completion listener each close an open popup and return focus to MetaStage while leaving its date unchanged. Park focus on a stable actor and restore the corresponding day/header after rebuilding, without stealing external focus; cover settings changes and asynchronous locale completion. |
