...
Audit ledger for this applet. Full-source rescan on 2026-07-12 (fresh pass) against every `agent-instructions/` review category **including the special ones — wiring gaps, unused functions/methods, legacy/deprecation, and variable/function scope**. Run blind: eight parallel scanners (security/data-governance, reliability/state-machines, wiring-gaps/dead-code/scope, architecture/duplication, testing/fuzzing, UI/accessibility/i18n, performance/memory, docs/packaging/release) each read the source **without this ledger**, and their findings were merged and deduplicated afterwards.
